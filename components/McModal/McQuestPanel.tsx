      ...
      <div className="flex-grow overflow-y-auto custom-scrollbar space-y-6 pr-1">
         {isEditing ? (
           <div className="space-y-4">
             ...
             <div className="p-3 bg-yellow-500/5 border border-yellow-500/20 rounded-sm space-y-3">
               <div className="flex items-center justify-between">
                 <span className="text-[10px] text-yellow-500 font-black uppercase tracking-widest">Cấu hình Hệ Thống</span>
                 <div className="flex items-center gap-2">
                    <span className="text-[8px] text-neutral-500 uppercase font-bold">Trạng thái:</span>
                    <button 
                      onClick={() => onUpdatePlayer?.({ systemName: player?.systemName ? "" : "Hệ Thống Vạn Giới" })}
                      className={`px-2 py-0.5 rounded-sm text-[8px] font-black uppercase transition-all ${player?.systemName ? 'bg-yellow-500 text-black' : 'bg-white/5 text-neutral-500 border border-white/10'}`}
                    >
                      {player?.systemName ? 'ĐANG BẬT' : 'ĐANG TẮT'}
                    </button>
                 </div>
               </div>
               
               {player?.systemName !== undefined && player?.systemName !== "" && (
