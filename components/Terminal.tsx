...
export const Terminal: React.FC<TerminalProps> = (props) => {
  const { 
    logs, onCommand, onOpenAiHint, onRetry, onRegenerateImage, isLoading, loadingProgress = 0, loadingStep = '',
            ...
            <div key={actualIndex} ref={isLast ? lastLogRef : (isLastPlayer ? currentTurnRef : null)} className={`animate-in fade-in slide-in-from-bottom-2 duration-500 ${log.type === 'player' ? 'text-emerald-400' : log.type === 'error' ? 'text-rose-400' : 'text-neutral-300'}`}>
              {log.type === 'player' && (
                <div className="flex items-center gap-3 mb-3 mt-4">
                   ...
                   <div className="px-3 py-1 bg-emerald-500/5 border border-emerald-500/20 rounded-full">
                      <span className="text-[10px] mono font-black text-emerald-500/90 uppercase tracking-[0.2em] whitespace-nowrap">HÀNH_ĐỘNG_{turnNumber.toString().padStart(3, '0')}</span>
                   </div>
                   <div className="h-px flex-grow bg-emerald-500/20"></div>
                </div>
              )}
              <div className="flex gap-3">
                ...
                <div className="flex-grow flex flex-col min-w-0">
                  ...
                  {log.imageUrl && (
                    <div className="mt-4 mb-2 rounded-xl overflow-hidden border border-white/10 shadow-2xl animate-in fade-in zoom-in duration-1000 max-w-2xl relative group/img">
                      <img 
                        src={log.imageUrl} 
