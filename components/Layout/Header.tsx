...
export const Header: React.FC<HeaderProps> = (props) => {
  const { 
    player, gameTime, currentLocation, isSaving, modals, setModals, handleBack, handleExit, view, onManualSave, onRegenerateImage, isLoading, settings, onUpdateSettings, markAsViewed 
  } = props;
      ...
      {isMobile && isMenuOpen && !isCollapsed && (
        <div className="absolute top-full left-0 w-full bg-black/95 backdrop-blur-3xl border-b border-white/10 p-4 grid grid-cols-2 gap-2 animate-in slide-in-from-top-4 duration-300 z-[60]">
          ...
          <button 
            ...
            className="px-3 py-4 bg-amber-500/10 border border-amber-500/40 text-amber-400 rounded-xl mono text-[10px] font-black uppercase"
          >
            🔄 Tải Lại
          </button>
          <button 
