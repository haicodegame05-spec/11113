...
export const MobileTerminal: React.FC<MobileTerminalProps> = ({ 
  logs, onCommand, onOpenAiHint, onRetry, onRegenerateImage, isLoading, placeholder = "Gõ hành động...", 
            ...
            <div key={actualIndex} ref={isLast ? lastLogRef : (isLastPlayer ? currentTurnRef : null)} className={`animate-in fade-in slide-in-from-bottom-1 duration-300 ${log.type === 'player' ? 'text-emerald-400' : log.type === 'error' ? 'text-rose-400' : 'text-neutral-300'}`}>
              {log.type === 'player' && (
                <div className="flex items-center gap-2 mb-2 mt-3">
                   ...
                   <div className="px-2 py-0.5 bg-emerald-500/5 border border-emerald-500/20 rounded-full">
                      <span className="text-[8px] mono font-black text-emerald-500/80 uppercase tracking-[0.2em] whitespace-nowrap">HÀNH_ĐỘNG_{turnNumber.toString().padStart(3, '0')}</span>
                   </div>
                   <div className="h-px flex-grow bg-emerald-500/20"></div>
                </div>
              )}
              <div className="flex gap-1.5">
                ...
                <div className="flex-grow flex flex-col min-w-0">
                  ...
                  {log.type === 'narrator' && onRegenerateImage && (
                    <div className="mt-3 mb-4">
                       <button 
                          ...
                          disabled={isLoading}
                          className="px-4 py-2 bg-emerald-500 text-black border border-emerald-400 rounded-lg active:bg-emerald-400 transition-all flex items-center gap-2 disabled:opacity-50 shadow-[0_0_20px_rgba(16,185,129,0.4)] active:scale-95"
                        >
                  ...
                  {log.imageUrl && (
                    <div className="mt-2 mb-1 rounded-xl overflow-hidden border border-white/10 shadow-lg animate-in fade-in zoom-in duration-700 relative group/img">
                      <img 
                        src={log.imageUrl} 
              ...
              <div className="flex justify-between items-center pt-1 border-t border-white/5 gap-2">
                {onRetry && (
                  <button 
                    ...
                    onClick={onRetry}
                    disabled={isLoading}
                    className={`flex-1 py-3 rounded-xl ${logs.length > 0 && logs[logs.length - 1].type === 'error' ? 'bg-rose-500/10 border-rose-500/20 text-rose-500' : 'bg-amber-500/10 border-amber-500/20 text-amber-500/60'} mono text-[11px] font-black uppercase tracking-wider flex items-center justify-center gap-2 active:scale-95 disabled:opacity-50 shadow-lg`}
                  >
                    <RefreshCw className={`w-4 h-4 ${isLoading ? 'animate-spin' : ''}`} />
                    Tải Lại
                  </button>
