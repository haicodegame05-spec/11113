...
import { GameLog, GameArchetype, Player, SubScenario, Relationship, GameTime, GameGenre, AppSettings, AiModel, CodexEntry, getGenreMeta, ThinkingLevel, StudioParams, WritingStyle, NarrativePerspective } from '../types';
...
export const useGameLogic = () => {
  ...
  const handleCommand = useCallback(async (command: string, timeCost?: number) => {
    ...
    try {
      ...
      finalNarratorText += `\n\n[ THỜI GIAN KIẾN TẠO THỰC TẠI: ${duration} GIÂY${tokenInfo} | ${coreLabel}${modelLabel}${proxyLabel} ]`;
      ...
      if (settings.autoGenerateImages) {
        ...
        try {
          ...
          const imagePrompt = `A high quality ${style} illustration for a text-based game. Scene description: ${narratorText}. Genre: ${selectedWorld.genre}. SFW, high detail, cinematic lighting, masterpiece. Negative prompt: nsfw, nude, naked, deformed, blurry, low quality, bad anatomy, extra limbs, text, watermark`;
          const imageUrl = await gameAI.generateImage(imagePrompt, settings);
          if (imageUrl) {
            finalLogsWithImage = finalLogsWithImage.map((l, idx) => 
              idx === finalLogsWithImage.length - 1 ? { ...l, imageUrl } : l
            );
            setLogs(finalLogsWithImage);
            
            // Add to player gallery for this world
            updatedPlayer.gallery = [{ url: imageUrl, tags: ['Mô Tả Truyện'], genre: selectedWorld.genre }, ...updatedPlayer.gallery];
          }
        } catch (err) {
      ...
      memoryService.updateMemory(finalLogsWithImage, updatedPlayer.turnCount);
      triggerAutoSave({ 
  ...
  const handleRegenerateImage = useCallback(async (logIndex: number) => {
    ...
    try {
      ...
      const style = styleMap[settings.imageStyle || ''] || styleMap['Ảnh chụp'];
      const imagePrompt = `A high quality ${style} illustration for a text-based game. Scene description: ${log.content}. Genre: ${selectedWorld.genre}. SFW, high detail, cinematic lighting, masterpiece. Negative prompt: nsfw, nude, naked, deformed, blurry, low quality, bad anatomy, extra limbs, text, watermark`;
      
      const imageUrl = await gameAI.generateImage(imagePrompt, settings);
      
      if (imageUrl) {
        const newLogs = logs.map((l, idx) => 
          idx === logIndex ? { ...l, imageUrl } : l
        );
        
        setLogs(newLogs);

        // Add to player gallery
        setPlayer(prev => ({
          ...prev,
          gallery: [{ url: imageUrl, tags: ['Mô Tả Truyện'], genre: selectedWorld.genre }, ...prev.gallery]
        }));
      ...
      const errorLog: GameLog = {
        content: `GM: Phác họa hình ảnh thất bại. [ LÝ DO: ${err.message || 'Lỗi không xác định'} ]`,
        type: 'error',
        timestamp: Date.now()
      };
  ...
  const handleStartGame = useCallback(async (scenarioText: string) => {
    if (!selectedWorld) return;
    
    const worldWithInstanceId = { ...selectedWorld, instanceId: generateMatrixId() };
    ...
    const finalGender = player.gender !== '??' ? player.gender : '??';
...
4. Hãy bắt đầu câu chuyện ngay lập tức dựa trên kịch bản và nhân vật này.`;
  ...
  return {
    ...
    handleStartFanficGame, handleRetry,
    ...
    generateAiAvatar: async (customPrompt?: string | StudioParams, npcId?: string) => {
      ...
      try {
        ...
        if (typeof customPrompt === 'object' && customPrompt !== null) {
          ...
          finalPrompt = `A high quality ${style} ${camera} of a character named ${name}. Gender: ${gender}. Age: ${age}. Personality: ${personality}. Genre: ${selectedWorld.genre}. Background: ${location}. ${lighting}. ${params.keywords}. SFW, high detail, masterpiece.`;
          ...
          const style = styleMap[settings.imageStyle || ''] || styleMap['Ảnh chụp'];
          finalPrompt = (customPrompt as string) || `A high quality ${style} portrait of a character named ${name}. Gender: ${gender}. Age: ${age}. Personality: ${personality}. Genre: ${selectedWorld.genre}. Background: ${location}. SFW, high detail, cinematic lighting, masterpiece.`;
          finalPrompt += ` Negative prompt: nsfw, nude, naked, deformed, blurry, low quality, bad anatomy, extra limbs, text, watermark`;
        }
        
        const imageUrl = await gameAI.generateImage(finalPrompt, settings, aspectRatio);
        if (imageUrl) {
          if (npcId) {
            setPlayer(prev => ({
              ...prev,
              relationships: prev.relationships.map(r => r.id === npcId ? { ...r, avatar: imageUrl } : r),
              gallery: [{ url: imageUrl, tags: ['avatar', name, 'npc'], genre: selectedWorld.genre }, ...prev.gallery]
            }));
          } else {
            setPlayer(prev => ({
              ...prev,
              avatar: imageUrl,
              gallery: [{ url: imageUrl, tags: ['avatar', name], genre: selectedWorld.genre }, ...prev.gallery]
            }));
          }
          return imageUrl;
        }
